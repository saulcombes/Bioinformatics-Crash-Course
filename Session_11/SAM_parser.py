...
def cigar_interpret(q,reference):
    cigarl= []
    ...
    for qname, flag,rname,pos,mapq,cigar,seq in q:
        ...
        for char in cigar:
            ...
            if char.isdigit():
                ...
                num = ""
        cigarl.append(cigarT)
    return cigarl

def walk_cigar(cigar_list,seq,ref,start_pos):
    ref_i = start_pos
    read_i = 0
    events = []
    ...
    for length,op in cigar_list:
            ...
            if op == "M":
                for _ in range(length):
                    events.append((ref_i, seq[read_i]))
                    ref_i += 1
                    read_i += 1
                
            elif op == "I":
                for _ in range(length):
                    events.append(("INS",seq[read_i]))
                    read_i += 1
...
cigarl = cigar_interpret(q,reference)
...
for read in cigarl:
    for qname,flag,rname,pos,mapq,cigar,seq in q:
        ...
        events = walk_cigar(read,seq,reference,pos)
        print(events)
        break
